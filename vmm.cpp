...
void print_page_table(std::ofstream &outfile) {
    ...
    for (const auto &entry : PageTable) {
        ...
        outfile << std::setw(4) << page_number << " - "
                << std::setw(5) << frame_number << " - " << std::setw(9)
...
void print_TLB(std::ofstream &outfile) {
    ...
    for (const auto &entry : TLB) {
        ...
        outfile << std::setw(4) << page_number << " - "
                << std::setw(5) << frame_number << std::endl;
    }
    outfile << "************\n";
}
...
int check_page_table(int page_number) {
    if (PageTable.find(page_number) != PageTable.end() && PageTable[page_number].valid)
...
void translate_address(int logical_address, const std::string &replacement_method, std::ofstream &outfile) {
...
int main(int argc, char *argv[]) {
    ...
    if (replacement_method != "fifo" && replacement_method != "lru") {
        std::cerr << "Invalid replacement method. Use 'fifo' or 'lru'." << std::endl;
        return 1;
    }
